...
def train(config):
    ...
    for num_iter in tqdm(range(config['max_iter'])):
        ...
        total_loss = classifier_loss + loss_params['domain_off'] * transfer_loss \
                     - loss_params['class_off'] * inconsistency_loss
        total_loss.backward()
        optimizer.step()
        ...
        if num_iter % config['val_iter'] == 0:
            print('class:', classifier_loss.item(),
                  ...
                  'inconsistency:', inconsistency_loss.item() * loss_params['class_off'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--net', type=str, default='ResNet50', help='Options: ResNet18,34,50,101,152')
    parser.add_argument('--data_set', type=str, default='clef', help='Options: office,clef,visda')
    parser.add_argument('--source_path', type=str, default='../data/clef/i_list.txt', help='The source list')
    parser.add_argument('--target_path', type=str, default='../data/clef/p_list.txt', help='The target list')
    parser.add_argument('--max_iter', type=int, default=7501, help='max iterations')
    ...
    config = {'seed': args.seed, 'gpu': args.gpu, 'max_iter': args.max_iter, 'val_iter': args.val_iter,
              ...
              'loss': {'domain_off': 1.0, 'class_off': 1.0},
              'prep': {'test_10crop': True, 'params': {'resize_size': 256, 'crop_size': 224}},
              'network': {'name': network.ResBase, 'params': {'resnet_name': args.net, 'class_num': 12}},
